...
function Download-Repo {
    ...
    $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("a1zap-install-" + [guid]::NewGuid().ToString("N"))
...
Write-Host ""
Write-Host "==============================================="
Write-Host "     A1Zap MiniApp Editor - Windows Installer"
Write-Host "==============================================="
Write-Host ""
...
$homeDir = if ($env:A1ZAP_INSTALL_HOME) { $env:A1ZAP_INSTALL_HOME } else { [Environment]::GetFolderPath("UserProfile") }
$installRoot = Join-Path $homeDir ".a1zap"
$installDir = Join-Path $installRoot "cli"
$binDir = Join-Path $installRoot "bin"
$shimPath = Join-Path $binDir "a1zap.cmd"
...
Write-Step "Downloading A1Zap CLI..."
...
$installLog = Join-Path ([System.IO.Path]::GetTempPath()) ("a1zap-install-" + [guid]::NewGuid().ToString("N") + ".log")
...
if ((Test-Path (Join-Path $installDir "tsconfig.json")) -and -not (Test-Path (Join-Path $installDir "dist"))) {
    Write-Step "Building..."
    $buildLog = Join-Path ([System.IO.Path]::GetTempPath()) ("a1zap-build-" + [guid]::NewGuid().ToString("N") + ".log")
    Run-OrFail -Label "Build" -LogFile $buildLog -Command { npm run build --silent }
    Write-Ok "Built"
}
Pop-Location

if (-not (Test-Path (Join-Path $installDir "bin\a1zap.js")) -or -not (Test-Path (Join-Path $installDir "dist\cli.js"))) {
    Fail "Installation is incomplete. Expected CLI files were not created."
}

New-Item -ItemType Directory -Path $binDir -Force | Out-Null
$shimContents = "@echo off`r`nnode `"%~dp0..\cli\bin\a1zap.js`" %*`r`n"
Set-Content -Path $shimPath -Value $shimContents -Encoding ASCII
Write-Ok "CLI installed"

if (-not $env:A1ZAP_SKIP_PATH_UPDATE) {
...
Write-Host '     a1zap config "your-api-key"'
Write-Host ""
Write-Host "  3. List your apps:"
Write-Host "     a1zap list"
Write-Host ""
